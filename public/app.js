...
const translations = {
  zh: {
    ...
    clickTag: "点击左侧标签",
    modelSettings: "模型设置",
    apiKeyLocal: "API Key 仅保存在本机 data/config.json",
    defaultModel: "默认模型",
    saveSettings: "保存设置",
    testModel: "测试模型",
    noPapers: "暂无论文",
  ...
  en: {
    ...
    apiKeyLocal: "API keys are stored locally in data/config.json",
...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(payload.error || (state.language === "en" ? `Request failed: ${response.status}` : `请求失败：${response.status}`));
  return payload;
}
