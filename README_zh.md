...
**2. 打标签，不怕乱也不怕忘。** 输标签时自动联想库里已有的，免得同一个概念起三个名字。没打标签的自动进内置的**「待归类」**，不会存完就石沉大海。

**3. 主题包：按标签组合出专题。** 不是简单的标签筛选，而是一条实时的查询：*含 A、含 B、排除 C*。比如做一个「具身智能里跟导航有关、但不要纯仿真的」，之后新存的论文只要符合条件就自动进这个包。

**4. 检索：本地秒搜 + 大模型语义搜。** 本地全文搜标题、摘要、备注、剪藏正文，不联网、不花钱，只记得论文里某一句话也能搜出来。想模糊找的时候切到 LLM 智能检索，用大白话描述你要什么就行。
...
智能检索、相似论文推荐、标签合并建议这三个需要模型。去**管理页 → 模型设置**填一个：
