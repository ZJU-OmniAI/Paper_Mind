...
Tag descriptions: with a configured model key and automatic descriptions enabled (the default), saving papers or updating tags may send each tag name, aliases, and bounded excerpts from up to six linked papers to the selected provider. Each paper contributes at most 2,400 characters of abstract, 1,200 characters of notes and 3,600 characters of clip text. Model replies, model names and generation timestamps are stored locally. Automatic descriptions can be disabled in Settings; manual generation remains available. API keys are excluded from JSON exports.
