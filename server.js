...
async function ensureFiles() {
  ...
  await writeJsonIfMissing(STORE_FILE, DEFAULT_STORE);
  await writeJsonIfMissing(CONFIG_FILE, DEFAULT_CONFIG, 0o600);
}

async function writeJsonIfMissing(file, value, mode) {
  try {
...
function findExistingTag(tags, name, mergeSuggestions = []) {
  const key = tagKey(name);
  const exact = tags.find((tag) => tagKey(tag.name) === key || tag.aliases?.some((alias) => tagKey(alias) === key));
  if (exact) return exact;

  const suggested = mergeSuggestions.find((item) => tagKey(item.incoming) === key && item.existing);
  if (suggested && Number(suggested.confidence || 0) >= 0.78) {
    const match = tags.find((tag) => tagKey(tag.name) === tagKey(suggested.existing));
...
function ensureTag(store, name, source, meta = {}, mergeSuggestions = []) {
  const cleanName = normalizeTagName(name);
  if (!cleanName) return null;
  const existing = findExistingTag(store.tags, cleanName, mergeSuggestions);
...
function linkTags(store, child, parentName, relatedNames = []) {
  if (!child) return;
  const parentClean = normalizeTagName(parentName);
  if (parentClean && tagKey(parentClean) !== tagKey(child.name)) {
    const parent = findTagByName(store, parentClean);
  ...
  for (const relatedName of relatedNames || []) {
    const relatedClean = normalizeTagName(relatedName);
    if (!relatedClean || tagKey(relatedClean) === tagKey(child.name)) continue;
    const related = findTagByName(store, relatedClean);
    if (related && related.id !== child.id) {
      child.relatedIds = uniq([...(child.relatedIds || []), related.id]);
      related.relatedIds = uniq([...(related.relatedIds || []), child.id]);
    }
  }
}
...
function fallbackTagsForPaper(input, existingTags) {
  const text = `${input.title || ""} ${input.abstract || ""} ${input.conversation || ""}`.toLowerCase();
  const rules = [
    ["大语言模型", ["llm", "large language model", "大语言模型", "语言模型", "gpt", "qwen", "glm"]],
    ["检索增强生成", ["rag", "retrieval augmented", "检索增强", "知识检索"]],
    ["智能体", ["agent", "智能体", "multi-agent", "工具调用"]],
    ["信息检索", ["retrieval", "search", "检索", "搜索"]],
    ["知识图谱", ["knowledge graph", "知识图谱", "graph"]],
    ["推理", ["reasoning", "推理", "chain-of-thought", "cot"]],
    ["多模态", ["multimodal", "vision-language", "多模态", "视觉语言"]],
    ["机器学习", ["machine learning", "deep learning", "神经网络", "训练"]],
    ["自然语言处理", ["nlp", "natural language", "文本", "语言"]],
    ["论文阅读", ["survey", "综述", "benchmark", "评测"]]
  ];
  const matched = rules
    .filter(([, keys]) => keys.some((key) => text.includes(key)))
    .map(([name]) => ({ name, reason: "本地关键词规则匹配", confidence: 0.55, related: [] }));

  for (const tag of existingTags) {
    if (text.includes(tag.name.toLowerCase())) {
      matched.push({ name: tag.name, reason: "命中已有标签名称", confidence: 0.65, related: [] });
    }
  }

  if (!matched.length) matched.push({ name: "待分类", reason: "未配置模型或模型不可用时的临时标签", confidence: 0.3, related: [] });
  return uniq(matched.map((tag) => tag.name)).slice(0, 8).map((name) => matched.find((tag) => tag.name === name));
}
...
async function callLLM(config, messages, { temperature = 0.2, json = true, returnMeta = false } = {}) {
  ...
  if (provider === "zhipu") {
    requestBody.max_tokens = ZHIPU_MAX_TOKENS;
  }
...
async function recommendTagsWithLLM(config, input, store) {
  const existingTags = store.tags.map((tag) => ({
    name: tag.name,
    aliases: tag.aliases || [],
    parents: (tag.parentIds || []).map((id) => store.tags.find((item) => item.id === id)?.name).filter(Boolean),
    related: (tag.relatedIds || []).map((id) => store.tags.find((item) => item.id === id)?.name).filter(Boolean)
  }));
  ...
  const messages = [
    {
      role: "system",
      content:
        "你是论文标签知识库管理员。请为论文推荐中文为主、必要时包含英文术语的研究标签，并整理标签间的父子和相关关系。只返回 JSON，不要 Markdown。"
    },
    {
      role: "user",
      content: JSON.stringify({
        task: "recommend_paper_tags",
        schema: {
          tags: [
            {
              name: "标签名",
          ...
          mergeSuggestions: [
            {
              incoming: "新标签",
              existing: "已有标签",
              relation: "same|parent|child|related",
              confidence: 0.9,
              reason: "合并或关联原因"
            }
          ]
        ...
        paper: {
          title: compactText(input.title, 400),
          abstract: compactText(input.abstract, 2200),
          conversation: compactText(input.conversation, 2200),
          manualTags: splitManualTags(input.manualTags)
        },
        existingTags
      })
    }
  ...
  return {
    ...
    mergeSuggestions: Array.isArray(result.mergeSuggestions) ? result.mergeSuggestions : []
  };
}

async function searchTagsWithLLM(config, query, store) {
  const tagCatalog = store.tags.map((tag) => ({
    id: tag.id,
    name: tag.name,
    aliases: tag.aliases || [],
  ...
  const messages = [
    {
      role: "system",
      content:
        "你是论文标签检索助手。根据用户关键词，从标签库中选择最相关标签，并说明理由。只返回 JSON，不要 Markdown。"
    },
    {
      role: "user",
      content: JSON.stringify({
...
async function mergeSimilarTagsWithLLM(config, store) {
    ...
    },
    {
      ...
      content: JSON.stringify({
        ...
        rules: [
          ...
          "只合并语义十分相似、几乎同义、或中英文/缩写表达同一概念的标签，例如 RAG、Retrieval-Augmented Generation、检索增强生成。",
          "合并时 canonical 是保留的规范标签名，tags 是要合并进 canonical 的已有标签名。",
          "不要把只是相关但不是同义的标签合并，例如 知识图谱 和 RAG 不能合并；模型训练 和 训练策略 不能合并，除非标签库语境明确完全等价。",
  ...
  const result = await callLLM(config, messages, { temperature: 0.1 });
  return {
...
async function handleApi(req, res, url) {
  const store = await readJson(STORE_FILE, DEFAULT_STORE);
  ensureStoreMeta(store);
  const config = { ...DEFAULT_CONFIG, ...(await readJson(CONFIG_FILE, DEFAULT_CONFIG)) };

  if (req.method === "GET" && url.pathname === "/api/state") {
    return sendJson(res, 200, { ...store, config: publicConfig(config) });
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/settings") {
    ...
    await writeJson(CONFIG_FILE, nextConfig, 0o600);
    return sendJson(res, 200, { config: publicConfig(nextConfig) });
  }

  if (req.method === "POST" && url.pathname === "/api/test-model") {
    const body = await readBody(req);
    const question = String(body.question || "").trim();
    if (!question) return sendJson(res, 400, { error: "测试问题不能为空" });
    const testConfig = {
      ...config,
      provider: body.provider === "zhipu" ? "zhipu" : "qwen",
      qwenModel: body.qwenModel || config.qwenModel,
    ...
    return sendJson(res, 200, { answer: result.content, meta: result.meta });
  }

  if (req.method === "POST" && url.pathname === "/api/recommend-tags") {
    return sendJson(res, 410, { error: "已关闭论文自动标签生成。请人工输入标签，再使用 LLM 整理标签库。" });
  }

  if (req.method === "POST" && url.pathname === "/api/papers") {
    const body = await readBody(req);
    const title = String(body.title || "").trim();
    if (!title) return sendJson(res, 400, { error: "论文标题不能为空" });
    ...
    return sendJson(res, 201, { paper, tags: store.tags, meta: store.meta });
  }

  const paperSimilarMatch = url.pathname.match(/^\/api\/papers\/([^/]+)\/similar-llm$/);
  if (req.method === "POST" && paperSimilarMatch) {
    const paperId = decodeURIComponent(paperSimilarMatch[1]);
    if (!store.papers.some((paper) => paper.id === paperId)) return sendJson(res, 404, { error: "论文不存在" });
    let llmUsed = true;
    let error = "";
    let matches = [];
    try {
    ...
    return sendJson(res, 200, { matches, llmUsed, error });
  }

  const paperMatch = url.pathname.match(/^\/api\/papers\/([^/]+)$/);
  if (req.method === "PUT" && paperMatch) {
    const body = await readBody(req);
    const paper = store.papers.find((item) => item.id === paperMatch[1]);
    ...
    await writeJson(STORE_FILE, store);
    return sendJson(res, 200, { paper, tags: store.tags, meta: store.meta });
  }

  if (req.method === "POST" && url.pathname === "/api/search") {
    const body = await readBody(req);
    const query = String(body.query || "").trim();
    if (!query) return sendJson(res, 400, { error: "检索关键词不能为空" });
    let llmUsed = true;
    let error = "";
    let matches = [];
    try {
  ...
  if (req.method === "POST" && url.pathname === "/api/tags") {
    const body = await readBody(req);
    const tag = ensureTag(store, body.name, "manual", { description: body.description || "" });
    if (!tag) return sendJson(res, 400, { error: "标签名不能为空" });
    linkTags(store, tag, body.parent, splitManualTags(body.related));
    markTagsStale(store);
    await writeJson(STORE_FILE, store);
    return sendJson(res, 201, { tag, tags: store.tags, meta: store.meta });
  }

  const tagMatch = url.pathname.match(/^\/api\/tags\/([^/]+)$/);
  if (req.method === "DELETE" && tagMatch) {
    const tagId = decodeURIComponent(tagMatch[1]);
    const tag = store.tags.find((item) => item.id === tagId);
    if (!tag) return sendJson(res, 404, { error: "标签不存在" });
    ...
    markTagsStale(store);
    await writeJson(STORE_FILE, store);
    return sendJson(res, 200, { deletedTagId: tagId, papers: store.papers, tags: store.tags, meta: store.meta });
  }

  if (req.method === "POST" && url.pathname === "/api/tags/rebuild") {
    let llmUsed = true;
    let error = "";
    let result;
    try {
    ...
    return sendJson(res, 200, { papers: store.papers, tags: store.tags, meta: store.meta, llmUsed, error, merges });
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/tags/merge") {
    ...
    markTagsStale(store);
    await writeJson(STORE_FILE, store);
    return sendJson(res, 200, { papers: store.papers, tags: store.tags, meta: store.meta, mergeCount });
  }

  if (req.method === "POST" && url.pathname === "/api/tags/curation-reviewed") {
    const body = await readBody(req);
    markTagsCurated(store, config, Number(body.mergeCount || 0));
    await writeJson(STORE_FILE, store);
    return sendJson(res, 200, { papers: store.papers, tags: store.tags, meta: store.meta });
  }
